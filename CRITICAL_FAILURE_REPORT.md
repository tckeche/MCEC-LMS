...
## Sign-in verification blocked by DB reachability (attempts 1-2)
- **Files touched:** None.
- **Error message / stack trace:**
  - `Login error: AggregateError [ENETUNREACH]: connect ENETUNREACH 3.147.243.31:5432` (and other IPs) from `DatabaseStorage.getUserByEmail` in `server/auth.ts`.
  - `Staff login error: AggregateError [ENETUNREACH]` from `DatabaseStorage.getUserByEmail` in `server/auth.ts`.
- **Failing API request + response:**
  - `POST http://localhost:5000/api/auth/login` with `{ "email": "student1@mcec.com", "password": "Student1" }` → `HTTP/1.1 500 Internal Server Error` `{ "message": "Login failed" }`.
  - `POST http://localhost:5000/api/auth/staff-login` with `{ "email": "tutor1@melaniacalvin.com", "password": "Tutorone" }` → `HTTP/1.1 500 Internal Server Error` `{ "message": "Login failed" }`.
- **Attempts:**
  1. Started server with `SESSION_SECRET=dev-secret` and production DB URL; student login returned 500 with ENETUNREACH DB errors.
  2. Same server instance; staff login returned 500 with ENETUNREACH DB errors.
